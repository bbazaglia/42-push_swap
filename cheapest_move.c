...
void	do_cheapest_move(t_stack_list *stack_a, t_stack_list *stack_b)
{
	...
	while (cur_b)
	{
			...
			+ ft_abs(cur_b->cost_b) < ft_abs(cheapest_cost))
		{
			...
			cheapest_cost_b = cur_b->cost_b;
		}
		cur_b = cur_b->next;
	}
	do_move(stack_a, stack_b, cheapest_a, cheapest_cost_b);
}

void	do_move(t_stack_list *stack_a, t_stack_list *stack_b, int cheapest_a,
		int cheapest_cost_b)
{
	if (cheapest_a > 0 && cheapest_cost_b > 0)
	{
		while (cheapest_a-- > 0 && cheapest_cost_b-- > 0)
			rr(stack_a, stack_b);
	}
	else if (cheapest_a < 0 && cheapest_cost_b < 0)
	{
		while (cheapest_a++ < 0 && cheapest_cost_b++ < 0)
			rrr(stack_a, stack_b);
	}
